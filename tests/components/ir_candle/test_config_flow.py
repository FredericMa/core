"""Tests for the IR Candle config flow."""
...
async def test_user_flow_success(
    ...
    assert result["title"] == "IR Candle via Test IR transmitter"
    assert result["data"] == {
        CONF_INFRARED_ENTITY_ID: MOCK_INFRARED_ENTITY_ID,
    }
    assert result["result"].unique_id == f"ir_candle_{MOCK_INFRARED_ENTITY_ID}"
...
@pytest.mark.parametrize(
    ...
    [
        (None, "IR Candle via Test IR transmitter"),
        ("Living Room IR Blaster", "IR Candle via Living Room IR Blaster"),
    ],
)
