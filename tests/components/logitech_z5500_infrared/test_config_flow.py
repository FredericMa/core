"""Tests for the Logitech Z-5500 Infrared config flow."""
...
async def test_user_flow_success(
    ...
    assert result["title"] == "Logitech Z-5500 via Test IR transmitter"
    assert result["data"] == {
        CONF_INFRARED_ENTITY_ID: MOCK_INFRARED_ENTITY_ID,
    }
    assert result["result"].unique_id == f"logitech_z5500_ir_{MOCK_INFRARED_ENTITY_ID}"
...
@pytest.mark.parametrize(
    ...
    [
        (None, "Logitech Z-5500 via Test IR transmitter"),
        ("AC IR emitter", "Logitech Z-5500 via AC IR emitter"),
    ],
)
