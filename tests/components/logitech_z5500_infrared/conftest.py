"""Common fixtures for the Logitech Z-5500 Infrared tests."""
...
from homeassistant.components.logitech_z5500_infrared import PLATFORMS
from homeassistant.components.logitech_z5500_infrared.const import (
...
def mock_config_entry() -> MockConfigEntry:
    ...
    return MockConfigEntry(
        ...
        title="Logitech Z-5500 via Test IR transmitter",
        data={
            CONF_INFRARED_ENTITY_ID: MOCK_INFRARED_ENTITY_ID,
        },
        unique_id=f"logitech_z5500_ir_{MOCK_INFRARED_ENTITY_ID}",
    )
...
@pytest.fixture
def mock_make_z5500_command() -> Generator[None]:
    """Patch make_z5500_command to return the Z5500Code directly.

    This allows tests to assert on the high-level code enum value
    rather than the raw NEC timings.
    """
    with patch(
        "homeassistant.components.logitech_z5500_infrared.entity.make_z5500_command",
...
async def init_integration(
    ...
    mock_make_z5500_command: None,
    platforms: list[Platform],
) -> MockConfigEntry:
    """Set up the Logitech Z-5500 Infrared integration for testing."""
    ...
    with patch("homeassistant.components.logitech_z5500_infrared.PLATFORMS", platforms):
